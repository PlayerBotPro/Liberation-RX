...
while {alive _static} do {
    ...
    if (_static_class in static_vehicles_AI) then {
        ...
        if (count crew _static > 0) then {
            ...
            if (_gunner getVariable ["GRLIB_is_prisoner", false]) exitWith { [_gunner, false] spawn F_ejectUnit };
            if (_gunner getVariable ["GRLIB_is_kamikaze", false]) exitWith { [_gunner, false] spawn F_ejectUnit };
